...
class PixelNeRFNet(torch.nn.Module):
    def __init__(self, conf, stop_encoder_grad=False):
        ...
        """
        super().__init__()
        self.encoder = make_encoder(conf["encoder"]) # feature extractor
        ...
        # Use Viewdirections d to positional encoding
    ...
    def encode(self, images, poses, focal, z_bounds=None, c=None):
        ...
        """
        self.num_objs = images.size(0) # batch size
        ...
        self.encoder(images) # image encoding
        rot = poses[:, :3, :3].transpose(1, 2)  # (B, 3, 3) Rotation Matrix
        trans = -torch.bmm(rot, poses[:, :3, 3:])  # (B, 3, 1) # Translation 
